...
//被调用
//对某一寄存器进行读写操作时，先以写命令方式写入寄存器地址，再以读数据/写数据方式写入相应内容
...
//LCD_FLAG_Using = 1;
	...
	/*
	LCD_WriteREG(MAMR, (LCD_ReadREG(MAMR) | Cursor_Direction_Vert));
	delay_100ns(5);
	for(i=0; i<row+1; i++){
		LCD_SetCursor(x+width*i, y);
		LCD_WriteCommand(MWCR);
		for(vp=0; vp<height*rank+1; vp++){
			//LCD_WriteCommand(MRCR);		   //预读指令，光标自增
			//DDRAM_data = LCD_ReadData();   //读出预读数据，光标又自增。。。

			//LCD_SetCursor(x+width*i, y+vp);
			//LCD_WriteCommand(MWCR);
			LCD_WriteData(0x80);
			delay_100ns(1); 
		}
...
void LCD_UpdateInfo(void)
{
	...
	//uint8_t WLCR_temp;
	//uint8_t MISC_temp;
	uint8_t apptop, appbottom, appleft, appright;
	//uint8_t FNCR_temp;
	...
	*/
	...
	//LCD_WriteREG(MAMR, MAMR_temp);
	//LCD_WriteREG(WLCR, WLCR_temp);
	//LCD_WriteREG(MISC, MISC_temp);
	LCD_SetAppwindow(apptop, appbottom, appleft, appright);
	//LCD_WriteREG(FNCR, FNCR_temp);
