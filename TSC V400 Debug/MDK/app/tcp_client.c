		...
		switch(appcmd->app_state){
			...
			case CLIENT_WAITING_FOR_CMD: {
				if(memcmp(appcmd->dataptr, cmd1, strlen(cmd1)) == 0){
					...
					if(mytime.tm_year<100&&mytime.tm_mon<12&&mytime.tm_mday<32&&mytime.tm_hour<24&&mytime.tm_min<60&&mytime.tm_sec<60&&mytime.tm_mon>0){
...
void tcp_client_request(void)
{
	...
	destip.addr = (uint32_t)192+(168<<8)+(1<<16)+(20<<24);
